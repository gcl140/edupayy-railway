...
from .models import Payment, StudentProfile, Invoice, PaymentMethod, FeeStructure, InvoiceFee
...
#         try:
#             base_url = request.build_absolute_uri('/billing/payment_success/')
#             # success_url = f"{base_url}?session_id={{CHECKOUT_SESSION_ID}}" 
#             EXCHANGE_RATE = Decimal('2500')  # 1 USD = 2500 TZS
#             domain = request.build_absolute_uri('/')[:-1]

#             # Convert TZS to USD
#             usd_amount = invoice.total_amount / EXCHANGE_RATE
#             # usd_cents = int(usd_amount * 100)  # Stripe expects cents as integer
#             usd_cents =  int(invoice.total_amount) # Stripe expects cents as integer
...
# def payment_status_check(request, invoice_id):
#     invoice = get_object_or_404(Invoice, pk=invoice_id)
#     try:
#         payment = Payment.objects.filter(invoice=invoice).latest('payment_date')
#         return redirect('payment_success') + f'?invoice_id={invoice_id}'
#     except Payment.DoesNotExist:
...
from .models import Payment  # Or Invoice, depending on what you want exported
...
from .models import StudentProfile, ParentRequest
from django.contrib import messages
from django.contrib.admin.views.decorators import staff_member_required
...
def student_dashboard(request):
    ...
    # return render(request, 'billing/student_dashboard.html', {'profile': profile})
...
#             else:
#                 # create the ParentRequest record
...
#                         f"wants you to confirm them as your child. "
#                         f"<a href=\"{request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'accept']))}\">Accept</a> | "
#                         f"<a href=\"{request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'reject']))}\">Reject</a>"
#                     ),
#                     is_public=False
#                 )
...
#                 # send an email
#                 send_mail(
#                     "Please confirm your child request",
#                     f"Hi {parent.get_full_name()},\n\n"
#                     f"{request.user.get_full_name()} has listed you as their parent on EduPayy.\n"
#                     f"Please click here to accept: {request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'accept']))}\n"
#                     f"Or here to reject: {request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'reject']))}\n\n"
#                     "Thanks,\nEduPayy Team",
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not parent:
                ...
                #     f"Hi {parent.get_full_name()},\n\n"
                #     f"{request.user.get_full_name()} has listed you as their parent on EduPayy.\n"
                #     f"Accept: {request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'accept']))}\n"
                #     f"Reject: {request.build_absolute_uri(reverse('respond_parent_request', args=[req.id, 'reject']))}\n\n"
                #     "Thanks,\nEduPayy Team",
                #     settings.DEFAULT_FROM_EMAIL,
                #     [parent.email]
                # )
